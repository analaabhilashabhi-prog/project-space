...
export default function DashboardSidebar({ teamNumber: teamNumberProp, currentMember, loggedInRoll, isLeader }) {
  ...
  useEffect(function () {
    async function checkStatus() {
      ...
      try {
        var foodRes = await supabase.from("food_selections").select("day_number").eq("member_roll_number", loggedInRoll)
        if (foodRes.data) {
          var days = new Set(foodRes.data.map(function (f) { return f.day_number }))
          setFoodDone(days.size >= 7)
        }
        var cartRes = await supabase.from("food_codes").select("id").eq("roll_number", loggedInRoll).limit(1)
        if (cartRes.data && cartRes.data.length > 0) setHasCart(true)
      } catch (e) {}
    }
    checkStatus()
  ...
  var navItems = [
    ...
    { id: "schedule", label: "Event Schedule", path: "/event-schedule", icon: "schedule" },
    { id: "snacks", label: "Food Cards", path: hasCart ? "/food-cards/" + teamNumber : "/food-selection/" + teamNumber, icon: "cards" },
  ]
  if (isLeader) {
    navItems.push({ id: "mentor", label: "Request Mentor", path: "/mentor-request/" + teamNumber, icon: "mentor" })
  }
  ...
  function renderIcon(iconId, size) {
    ...
    switch (iconId) {
      ...
      case "schedule":
        return (<svg viewBox="0 0 24 24" style={s}><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>)
      case "cards":
  ...
  return (
    <>
      <style jsx>{`
        .ns { width:68px; min-width:68px; height:100vh; background:linear-gradient(180deg,#0c0c0c 0%,#080808 50%,#0c0c0c 100%); border-right:1px solid rgba(255,255,255,0.04); display:flex; flex-direction:column; position:relative; z-index:50; overflow-y:auto; overflow-x:hidden; }
        .ns.ready { transition:width 0.32s cubic-bezier(0.4,0,0.2,1),min-width 0.32s cubic-bezier(0.4,0,0.2,1); }
        .ns::-webkit-scrollbar { width:0; }
        .ns.open { width:232px; min-width:232px; }
        .ns-head { display:flex; align-items:center; gap:10px; padding:16px 15px 14px; position:relative; cursor:pointer; user-select:none; }
        .ns-avatar { width:36px; height:36px; min-width:36px; border-radius:50%; background:#0a0a0a; border:2px solid rgba(255,96,64,0.25); display:flex; align-items:center; justify-content:center; overflow:hidden; transition:border-color 0.3s ease,transform 0.2s ease; }
        .ns-head:hover .ns-avatar { border-color:rgba(255,96,64,0.5); transform:scale(1.06); }
        .ns-avatar img { width:100%; height:100%; object-fit:cover; border-radius:50%; }
        .ns-avatar-placeholder { font-weight:700; font-size:14px; color:rgba(255,255,255,0.5); font-family:'DM Sans',sans-serif; }
        .ns-brand { font-family:'DM Sans',sans-serif; font-weight:700; font-size:15px; color:#fff; letter-spacing:1px; white-space:nowrap; opacity:0; width:0; overflow:hidden; }
        ...
        .ns-item { display:flex; align-items:center; gap:12px; padding:10px 0; justify-content:center; cursor:pointer; position:relative; margin-bottom:2px; color:rgba(255,255,255,0.7); }
        ...
        .ns-item.on::before { content:''; position:absolute; left:0; top:50%; transform:translateY(-50%); width:3px; height:20px; border-radius:0 3px 3px 0; background:linear-gradient(180deg,#ff3520,#ff7a40); box-shadow:0 0 8px rgba(255,55,25,0.4); }
        .ns-ic { width:20px; height:20px; min-width:20px; display:flex; align-items:center; justify-content:center; position:relative; }
        .ns.ready .ns-ic { transition:all 0.22s ease; }
        .ns-dot { position:absolute; top:-2px; right:-4px; width:7px; height:7px; border-radius:50%; background:#ff3b2e; border:1.5px solid #0c0c0c; box-shadow:0 0 5px rgba(255,59,46,0.5); animation:nsPulse 2s ease-in-out infinite; }
        @keyframes nsPulse { 0%,100%{opacity:1} 50%{opacity:0.4} }
        .ns-txt { font-size:13px; font-weight:500; white-space:nowrap; opacity:0; width:0; overflow:hidden; }
        ...
        .ns-badge { margin-left:auto; padding:1px 7px; border-radius:10px; background:rgba(255,50,25,0.12); color:#ff6040; font-size:10px; font-weight:700; font-family:'DM Sans',sans-serif; opacity:0; position:absolute; pointer-events:none; }
        .ns.ready .ns-badge { transition:opacity 0.2s ease; }
        .ns.open .ns-badge { opacity:1; position:relative; pointer-events:auto; }
        .ns-tip { position:absolute; left:calc(100% + 14px); top:50%; transform:translateY(-50%) translateX(-4px); background:rgba(20,20,20,0.95); color:#fff; font-size:12px; font-weight:600; font-family:'DM Sans',sans-serif; letter-spacing:0.8px; padding:6px 14px; border-radius:8px; white-space:nowrap; pointer-events:none; opacity:0; transition:opacity 0.18s ease,transform 0.18s ease; box-shadow:0 4px 20px rgba(0,0,0,0.6); border:1px solid rgba(255,96,64,0.12); z-index:100; }
        .ns-tip::before { content:''; position:absolute; left:-5px; top:50%; transform:translateY(-50%) rotate(45deg); width:10px; height:10px; background:rgba(20,20,20,0.95); border-left:1px solid rgba(255,96,64,0.12); border-bottom:1px solid rgba(255,96,64,0.12); }
        .ns-item:hover .ns-tip { opacity:1; transform:translateY(-50%) translateX(0); }
        .ns.open .ns-tip { display:none; }
        .ns-promo { margin:6px 10px 8px; padding:14px 12px; border-radius:13px; background:linear-gradient(155deg,rgba(255,50,25,0.09),rgba(255,90,50,0.03)); border:1px solid rgba(255,60,30,0.1); position:relative; overflow:hidden; max-height:200px; }
        .ns.ready .ns-promo { transition:all 0.32s ease; }
        .ns-promo::before { content:''; position:absolute; top:-15px; right:-15px; width:60px; height:60px; background:radial-gradient(circle,rgba(255,55,25,0.1),transparent 70%); pointer-events:none; }
        .ns:not(.open) .ns-promo { opacity:0; max-height:0; margin:0; padding:0; border:none; pointer-events:none; overflow:hidden; }
        .ns-promo-em { font-size:16px; margin-bottom:6px; }
        .ns-promo-t { font-size:12px; font-weight:700; color:#fff; margin-bottom:3px; }
        .ns-promo-t span { background:linear-gradient(135deg,#ff3520,#ff8040); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
        .ns-promo-s { font-size:10px; color:rgba(255,255,255,0.28); line-height:1.5; margin-bottom:10px; }
        .ns-promo-b { width:100%; padding:7px; border-radius:8px; border:none; background:linear-gradient(135deg,#ff3520,#ff6040); color:#fff; font-size:11px; font-weight:600; font-family:'DM Sans',sans-serif; cursor:pointer; transition:all 0.25s ease; box-shadow:0 2px 10px rgba(255,50,25,0.2); }
        .ns-promo-b:hover { box-shadow:0 4px 20px rgba(255,50,25,0.35); transform:translateY(-1px); }
        .ns-bottom { padding:8px 8px 14px; border-top:1px solid rgba(255,255,255,0.04); margin-top:auto; }
        .ns-logout { display:flex; align-items:center; gap:12px; padding:10px 0; justify-content:center; cursor:pointer; position:relative; color:rgba(255,255,255,0.4); }
        ...
        .ns-logout-ic { width:20px; height:20px; min-width:20px; display:flex; align-items:center; justify-content:center; }
        .ns-logout-txt { font-size:13px; font-weight:500; white-space:nowrap; opacity:0; width:0; overflow:hidden; }
        .ns.ready .ns-logout-txt { transition:opacity 0.2s ease; }
        .ns.open .ns-logout-txt { opacity:1; width:auto; }
        .ns-logout-tip { position:absolute; left:calc(100% + 14px); top:50%; transform:translateY(-50%) translateX(-4px); background:rgba(20,20,20,0.95); color:#fff; font-size:12px; font-weight:600; font-family:'DM Sans',sans-serif; letter-spacing:0.8px; padding:6px 14px; border-radius:8px; white-space:nowrap; pointer-events:none; opacity:0; transition:opacity 0.18s ease,transform 0.18s ease; box-shadow:0 4px 20px rgba(0,0,0,0.6); border:1px solid rgba(255,96,64,0.12); z-index:100; }
        .ns-logout-tip::before { content:''; position:absolute; left:-5px; top:50%; transform:translateY(-50%) rotate(45deg); width:10px; height:10px; background:rgba(20,20,20,0.95); border-left:1px solid rgba(255,96,64,0.12); border-bottom:1px solid rgba(255,96,64,0.12); }
        .ns-logout:hover .ns-logout-tip { opacity:1; transform:translateY(-50%) translateX(0); }
        .ns.open .ns-logout-tip { display:none; }
        .ns-overlay { display:none; position:fixed; inset:0; background:rgba(0,0,0,0.6); backdrop-filter:blur(3px); z-index:45; }
        .ns-overlay.vis { display:block; }
        .ns-hamburger { display:none; width:40px; height:40px; border-radius:10px; border:1px solid rgba(255,255,255,0.08); background:rgba(255,255,255,0.03); align-items:center; justify-content:center; cursor:pointer; color:#888; font-size:20px; position:fixed; top:14px; left:14px; z-index:44; }
        @media(max-width:768px) {
          .ns { position:fixed; left:-240px; z-index:50; width:232px; min-width:232px; transition:left 0.3s cubic-bezier(0.4,0,0.2,1); }
      ...
      <aside className={"ns " + (ready ? "ready " : "") + (expanded ? "open " : "") + (mobileMenuOpen ? "mob" : "")}>
        <div className="ns-head" onClick={function () { toggleExpanded() }}>
          <div className="ns-avatar">
            {userPhotoUrl ? <img src={userPhotoUrl} alt={userName} onError={function (e) { e.target.style.display = "none"; e.target.nextSibling.style.display = "flex" }} /> : null}
        ...
        <div className="ns-nav">
          {navItems.map(function (item) {
            ...
            return (
              <div key={item.id} className={"ns-item " + (active ? "on" : "")} onClick={function () { navigateTo(item) }} onMouseEnter={function () { setHoveredId(item.id) }} onMouseLeave={function () { setHoveredId(null) }}>
                <div className="ns-ic">

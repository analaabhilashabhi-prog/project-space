...
function blankMember(isLeader) {
  return { member_name: "", member_roll_number: "", member_email: "", member_phone: "", member_branch: "", member_year: "", member_college: "", is_leader: isLeader || false, lookupDone: false, lookupLoading: false, manualEntry: false }
}

export default function RegisterPage() {
  var router = useRouter()
  var [loading, setLoading] = useState(false)
  var [fetchingTeam, setFetchingTeam] = useState(false)
  ...
  var [teamFetched, setTeamFetched] = useState(false)

  // Step 1 — Project details (all blank, user fills manually)
  ...
  var [members, setMembers] = useState([blankMember(true)])
  ...
  selectedScopes.forEach(function (scopeId) {
    ...
    techs.forEach(function (t) {
      if (!seenTechNames[t.name]) { seenTechNames[t.name] = true; availableTechs.push(t) }
    })
  })

  // Load leader roll from session/URL on mount — NO auto-fetch of team data
  useEffect(function () {
  ...
  // "Get Team" button — fetch from registered_teams + students
  function handleGetTeam() {
    if (!leaderRoll) { toast.error("No leader roll found. Please login again."); return }
    setFetchingTeam(true)
    fetch("/api/get-team-members", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ rollNumber: leaderRoll }),
    })
      .then(function (res) { return res.json() })
      .then(function (data) {
        if (data.found && data.members) {
          setMembers(data.members.map(function (m) {
            return Object.assign({}, blankMember(m.is_leader), m, { lookupDone: true })
          }))
          setTeamFetched(true)
          toast.success("Team loaded! All fields are editable.")
        } else {
          toast.error(data.error || "No pre-registered team found for your roll number.")
        }
        setFetchingTeam(false)
      })
      .catch(function () { toast.error("Something went wrong"); setFetchingTeam(false) })
  }

  // Manual lookup for individually added members
  var lookupStudent = useCallback(function (rollNumber, index) {
      ...
      .then(function (data) {
        setMembers(function (prev) {
          return prev.map(function (m, i) {
            ...
            if (data.alreadyInTeam) { toast.error(data.message); copy.lookupDone = false; copy.member_roll_number = ""; copy.member_name = ""; copy.member_college = ""; copy.member_branch = ""; return copy }
            if (data.found && data.student) {
              copy.member_name = data.student.name; copy.member_college = data.student.college; copy.member_branch = data.student.branch; copy.member_phone = data.student.phone || ""; copy.member_email = rollNumber.toLowerCase() + "@outlook.com"; copy.manualEntry = false
              toast.success("Student found — all fields editable")
            } else { copy.manualEntry = true; toast.error("Student not found — fill manually") }
            return copy
          })
        })
      })
  ...
  function addMember() { if (members.length >= MAX_MEMBERS) return; setMembers(function (prev) { return prev.concat([blankMember(false)]) }) }
  ...
  function handleRollBlur(index) {
    ...
    if (!member.member_name.trim()) lookupStudent(roll, index)
  }
  ...
  function handleSubmit() {
    ...
    var scopeNames = selectedScopes.map(function (s) {
      if (s.startsWith("other:")) return s.replace("other:", "")
      var found = SCOPES.find(function (sc) { return sc.id === s }); return found ? found.name : s
    })
    fetch("/api/register", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ projectTitle: projectTitle, projectDescription: projectDescription, technologies: selectedTechs, projectScope: scopeNames, aiCapabilities: aiCapabilities || null, aiTools: selectedAiTools.length > 0 ? selectedAiTools : null, members: cleanMembers }),
    })
      .then(function (res) { return res.json() })
      .then(function (data) {
  ...
  return (
    <div className="ps-page">
      ...
      <style jsx>{`
        ...
        /* Get Team banner */
        .reg-get-team-banner{display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-radius:14px;border:1px solid rgba(255,60,30,0.2);background:rgba(255,60,30,0.04);margin-bottom:24px;position:relative;z-index:1;gap:16px}
        .reg-get-team-text{font-family:var(--font-display);font-size:12px;font-weight:600;color:rgba(255,255,255,0.5);letter-spacing:1px;line-height:1.5}
        .reg-get-team-text strong{color:#fff;display:block;font-size:13px;margin-bottom:2px}
        .reg-get-team-btn{padding:10px 20px;border-radius:10px;border:none;background:linear-gradient(135deg,#ff3020,#ff6040);color:#fff;font-family:var(--font-display);font-size:12px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;cursor:pointer;transition:all 0.3s;white-space:nowrap;display:flex;align-items:center;gap:8px;flex-shrink:0}
        .reg-get-team-btn:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 4px 16px rgba(255,50,30,0.3)}
        .reg-get-team-btn:disabled{opacity:0.5;cursor:not-allowed}
        .reg-fetched-badge{display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:6px;background:rgba(68,255,102,0.08);border:1px solid rgba(68,255,102,0.2);color:#44ff66;font-family:var(--font-display);font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase}
        ...
        .reg-confirm-overlay{position:fixed;inset:0;z-index:500;background:rgba(0,0,0,0.7);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:20px}
        ...
        .reg-success-icon{width:80px;height:80px;border-radius:50%;background:rgba(68,255,102,0.1);border:2px solid rgba(68,255,102,0.3);display:flex;align-items:center;justify-content:center;margin:0 auto 20px;font-size:36px}
        ...
        @media(max-width:768px){.reg-grid{grid-template-columns:1fr}.reg-header{flex-direction:column;gap:12px;align-items:flex-start}.reg-steps{gap:4px}.reg-step-label{display:none}.reg-section{padding:22px 18px}.reg-member{padding:18px 14px}.reg-nav{flex-direction:column}.reg-confirm-btns{flex-direction:column}.reg-scope-grid{grid-template-columns:1fr 1fr}.reg-tech-grid{grid-template-columns:repeat(auto-fill,minmax(130px,1fr))}.reg-get-team-banner{flex-direction:column;align-items:flex-start}.reg-get-team-btn{width:100%;justify-content:center}}
      ...
      {showConfirm && (
        <div className="reg-confirm-overlay">
          <div className="reg-confirm-card">
            ...
            <div className="reg-confirm-desc">Once registered, please make sure all details are correct.</div>
        ...
        <div className="reg-container">
          ...
          {registered && (
            <div className="reg-section">
              <div className="reg-success">
                ...
                <button className="ps-btn ps-btn-primary" style={{ padding: "14px 36px", fontSize: 15 }} onClick={function () { router.push("/food-selection/" + teamNumber) }}>Proceed to Food Selection →</button>
              </div>
            </div>
          )}
            ...
            <div className="reg-section">
              ...
              <div style={{ position: "relative", zIndex: 1 }}>
                <div className="reg-field">
                  <div className="reg-field-label"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Project Title *</div>
                ...
                <div className="reg-field">
                  <div className="reg-field-label"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>Project Description *</div>
                ...
                <div className="reg-field">
                  <div className="reg-field-label"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>Project Scope * <span className="reg-field-opt">(select one or more)</span></div>
                  {selectedScopes.length > 0 && (<div className="reg-selected-chips">{selectedScopes.map(function (s) { var label = s.startsWith("other:") ? s.replace("other:", "") : (SCOPES.find(function (sc) { return sc.id === s }) || {}).name || s; return <div key={s} className="reg-chip">{label}<button onClick={function () { setSelectedScopes(function (p) { return p.filter(function (x) { return x !== s }) }) }}>✕</button></div> })}</div>)}
                  <div className="reg-scope-grid">
                    {SCOPES.map(function (scope) { var isOn = selectedScopes.includes(scope.id); return (<div key={scope.id} className={"reg-scope-card " + (isOn ? "on" : "")} onClick={function () { toggleScope(scope.id) }}><div className="reg-scope-card-icon"><LineIcon d={scope.icon} size={16} color={isOn ? "#ff6040" : "rgba(255,255,255,0.3)"} /></div><div className="reg-scope-card-name">{scope.name}</div></div>) })}
                  </div>
                  <div style={{ marginTop: 8 }}>{!showOtherScope ? (<button type="button" onClick={function () { setShowOtherScope(true) }} className="reg-add-other"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> Add other scope</button>) : (<div className="reg-add-other-row"><input type="text" className="ps-input" value={otherScope} onChange={function (e) { setOtherScope(e.target.value) }} placeholder="Scope name" style={{ flex: 1, fontSize: 13 }} onKeyDown={function (e) { if (e.key === "Enter") { e.preventDefault(); addOtherScope() } }} /><button type="button" className="ps-btn ps-btn-primary ps-btn-sm" onClick={addOtherScope}>Add</button><button type="button" className="ps-btn ps-btn-secondary ps-btn-sm" onClick={function () { setShowOtherScope(false); setOtherScope("") }}>Cancel</button></div>)}</div>
                </div>
                <div className="reg-field">
                  <div className="reg-field-label"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>Technology Stack * <span className="reg-field-opt">(based on scope above)</span></div>
                  {selectedTechs.length > 0 && (<div className="reg-selected-chips">{selectedTechs.map(function (t) { return <div key={t} className="reg-chip">{t}<button onClick={function () { toggleTech(t) }}>✕</button></div> })}</div>)}
                  {selectedScopes.length === 0 ? (<div style={{ padding: "20px", textAlign: "center", color: "rgba(255,255,255,0.2)", fontSize: 13, border: "1px dashed rgba(255,255,255,0.06)", borderRadius: 12 }}>Select a project scope first</div>) : (<div className="reg-tech-grid">{availableTechs.map(function (tech) { var isOn = selectedTechs.includes(tech.name); return (<button key={tech.name} type="button" className={"reg-tech-btn " + (isOn ? "on" : "")} onClick={function () { toggleTech(tech.name) }}><LineIcon d={tech.icon} size={14} color={isOn ? "#ff6040" : "rgba(255,255,255,0.3)"} />{tech.name}</button>) })}</div>)}
                  <div style={{ marginTop: 8 }}>{!showOtherTech ? (<button type="button" onClick={function () { setShowOtherTech(true) }} className="reg-add-other"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> Add other technology</button>) : (<div className="reg-add-other-row"><input type="text" className="ps-input" value={otherTech} onChange={function (e) { setOtherTech(e.target.value) }} placeholder="Technology name" style={{ flex: 1, fontSize: 13 }} onKeyDown={function (e) { if (e.key === "Enter") { e.preventDefault(); addOtherTech() } }} /><button type="button" className="ps-btn ps-btn-primary ps-btn-sm" onClick={addOtherTech}>Add</button><button type="button" className="ps-btn ps-btn-secondary ps-btn-sm" onClick={function () { setShowOtherTech(false); setOtherTech("") }}>Cancel</button></div>)}</div>
                </div>
                <div className="reg-field">
                  <div className="reg-field-label"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2a4 4 0 0 0-4 4c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2 4 4 0 0 0-4-4zM8 8v2m8-2v2M6 14a6 6 0 0 0 12 0M9 18l-1 4m7-4l1 4"/></svg>AI Capabilities <span className="reg-field-opt">(optional)</span></div>
                  <textarea className="ps-textarea" value={aiCapabilities} onChange={function (e) { setAiCapabilities(e.target.value) }} rows={3} placeholder="e.g. AI-based resume analysis, chatbot for student queries..." />
                </div>
                <div className="reg-field">
                  <div className="reg-field-label"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 3l1.5 5.5L19 10l-5.5 1.5L12 17l-1.5-5.5L5 10l5.5-1.5z"/></svg>AI Tools <span className="reg-field-opt">(optional)</span></div>
                  {selectedAiTools.length > 0 && (<div className="reg-selected-chips">{selectedAiTools.map(function (t) { return <div key={t} className="reg-chip">{t}<button onClick={function () { toggleAiTool(t) }}>✕</button></div> })}</div>)}
                  <div className="reg-tech-grid" style={{ maxHeight: 200 }}>{AI_TOOLS.map(function (tool) { var isOn = selectedAiTools.includes(tool.name); return (<button key={tool.name} type="button" className={"reg-tech-btn " + (isOn ? "on" : "")} onClick={function () { toggleAiTool(tool.name) }}><LineIcon d={tool.icon} size={14} color={isOn ? "#ff6040" : "rgba(255,255,255,0.3)"} />{tool.name}</button>) })}</div>
                  <div style={{ marginTop: 8 }}>{!showOtherAiTool ? (<button type="button" onClick={function () { setShowOtherAiTool(true) }} className="reg-add-other"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> Add other AI tool</button>) : (<div className="reg-add-other-row"><input type="text" className="ps-input" value={otherAiTool} onChange={function (e) { setOtherAiTool(e.target.value) }} placeholder="AI tool name" style={{ flex: 1, fontSize: 13 }} onKeyDown={function (e) { if (e.key === "Enter") { e.preventDefault(); addOtherAiTool() } }} /><button type="button" className="ps-btn ps-btn-primary ps-btn-sm" onClick={addOtherAiTool}>Add</button><button type="button" className="ps-btn ps-btn-secondary ps-btn-sm" onClick={function () { setShowOtherAiTool(false); setOtherAiTool("") }}>Cancel</button></div>)}</div>
                </div>
              </div>
            </div>
          )}
          ...
          {!registered && currentStep === 2 && (
            <div className="reg-section">
              ...
              {/* GET TEAM BANNER */}
              <div className="reg-get-team-banner">
                <div className="reg-get-team-text">
                  <strong>Auto-fill your team</strong>
                  Click "Get Team" to load your pre-registered members from the database. All fields will be editable.
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                  {teamFetched && <span className="reg-fetched-badge">✓ Loaded</span>}
                  <button type="button" className="reg-get-team-btn" onClick={handleGetTeam} disabled={fetchingTeam}>
                    {fetchingTeam ? (
                      <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: "regSpin 0.8s linear infinite" }}><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Loading...</>
                    ) : (
                      <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> Get Team</>
                    )}
                  </button>
                </div>
              </div>
              ...
              <div style={{ position: "relative", zIndex: 1 }}>
                  ...
                  return (
                    <div key={index} className={"reg-member" + (isLeaderMem ? " leader" : "")}>
                      <div className="reg-member-header">
                        ...
                        {!isLeaderMem && (
                          <button type="button" className="reg-delete-btn" onClick={function () { deleteMember(index) }} title="Remove">
                      ...
                      <div className="reg-grid">
                        <div className="reg-field" style={{ marginBottom: 0 }}>
                          ...
                          <input className="ps-input" type="text" value={member.member_roll_number}
                            onChange={function (e) { handleMemberChange(index, "member_roll_number", e.target.value.toUpperCase()); handleMemberChange(index, "lookupDone", false) }}
                            onBlur={function () { if (!isLeaderMem) handleRollBlur(index) }}
                            placeholder="e.g. 22A31A0501"
                            readOnly={isLeaderMem}
                            style={isLeaderMem ? { opacity: 0.6 } : {}}
                          />
                          {member.lookupLoading && <span className="reg-lookup-badge reg-lookup-loading">🔍 Looking up...</span>}
                          {!member.lookupLoading && member.lookupDone && !member.manualEntry && <span className="reg-lookup-badge reg-lookup-found">✓ Found</span>}
                          {!member.lookupLoading && member.manualEntry && <span className="reg-lookup-badge reg-lookup-manual">📝 Manual</span>}
                        </div>
                        <div className="reg-field" style={{ marginBottom: 0 }}>
                          <label className="ps-label">Full Name *</label>
                          <input className="ps-input" type="text" value={member.member_name} onChange={function (e) { handleMemberChange(index, "member_name", e.target.value) }} placeholder="Full Name" />
                        </div>
                        <div className="reg-field" style={{ marginBottom: 0 }}>
                          <label className="ps-label">College *</label>
                          <input className="ps-input" type="text" value={member.member_college} onChange={function (e) { handleMemberChange(index, "member_college", e.target.value) }} placeholder="College Name" />
                        </div>
                        <div className="reg-field" style={{ marginBottom: 0 }}>
                          <label className="ps-label">Branch *</label>
                          <input className="ps-input" type="text" list={"branches-" + index} value={member.member_branch} onChange={function (e) { handleMemberChange(index, "member_branch", e.target.value) }} placeholder="Select or type branch" />
                          <datalist id={"branches-" + index}>
                            {BRANCHES.map(function (b) { return <option key={b} value={b} /> })}
                          </datalist>
                        </div>
                      </div>
                    </div>
                ...
                {members.length < MAX_MEMBERS && (
                  <button type="button" className="reg-add-member" onClick={addMember}>✚ Add Member {members.length + 1}</button>
                )}
              </div>
            </div>
          )}
            ...
            <div className="reg-section">
              ...
              <div style={{ position: "relative", zIndex: 1 }}>
                <div className="reg-review-card">
                  <div className="reg-review-header"><div className="reg-review-title">Project Details</div><button className="reg-review-edit" onClick={function () { setCurrentStep(1) }}>Edit</button></div>
                  ...
                  <div className="reg-review-row"><div className="reg-review-label">Scope</div><div className="reg-selected-chips" style={{ marginTop: 4, marginBottom: 0 }}>{selectedScopes.map(function (s) { var label = s.startsWith("other:") ? s.replace("other:", "") : (SCOPES.find(function (sc) { return sc.id === s }) || {}).name || s; return <div key={s} className="reg-chip" style={{ cursor: "default" }}>{label}</div> })}</div></div>
                  <div className="reg-review-row"><div className="reg-review-label">Technologies</div><div className="reg-selected-chips" style={{ marginTop: 4, marginBottom: 0 }}>{selectedTechs.map(function (t) { return <div key={t} className="reg-chip" style={{ cursor: "default" }}>{t}</div> })}</div></div>
                  ...
                  {selectedAiTools.length > 0 && <div className="reg-review-row"><div className="reg-review-label">AI Tools</div><div className="reg-selected-chips" style={{ marginTop: 4, marginBottom: 0 }}>{selectedAiTools.map(function (t) { return <div key={t} className="reg-chip" style={{ cursor: "default" }}>{t}</div> })}</div></div>}
                </div>
                <div className="reg-review-card">
                  <div className="reg-review-header"><div className="reg-review-title">Team Members ({members.length})</div><button className="reg-review-edit" onClick={function () { setCurrentStep(2) }}>Edit</button></div>
                    ...
                    return (
                      <div key={i} className="reg-review-member">
                        <div className="reg-review-member-left">
                          ...
                          <div><div className="reg-review-member-name">{m.member_name}</div><div className="reg-review-member-sub">{m.member_roll_number} • {m.member_branch}</div></div>
                        </div>
          ...
          {!registered && (
            <div className="reg-nav">
              ...
              {currentStep < 3 ? (
                ...
                <button className="ps-btn ps-btn-primary" style={{ flex: 1, opacity: canRegister ? 1 : 0.4 }} onClick={function () { if (canRegister) setShowConfirm(true) }} disabled={loading || !canRegister}>

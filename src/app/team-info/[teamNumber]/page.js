...
export default function TeamInfoPage() {
  ...
  useEffect(function () {
    ...
    async function load() {
      ...
      var mentorRes = await supabase.from("mentor_requests").select("id").eq("team_number", teamNumber)
  ...
  // Derive technology for MilestoneTimeline
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: "#000" }}>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ textAlign: "center" }}>
            ...
            <div style={{ fontFamily: "var(--font-primary,'Open Sans',sans-serif)", fontSize: 13, letterSpacing: 3, color: "rgba(255,255,255,0.3)", textTransform: "uppercase" }}>Loading Dashboard</div>
          </div>
        </div>
  ...
  if (!team) {
    return (
      <div style={{ display: "flex", minHeight: "100vh", background: "#000" }}>
        <DashboardSidebar teamNumber={teamNumber} currentMember={currentMember} loggedInRoll={loggedInRoll} isLeader={isLeader} />
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-primary,'Open Sans',sans-serif)", fontSize: 20, color: "rgba(255,255,255,0.3)" }}>Team Not Found</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#000", color: "#fff", fontFamily: "var(--font-primary,'Open Sans',sans-serif)" }}>
      <style jsx>{`
        ...
        .toggle-btn{font-family:var(--font-primary,'Open Sans',sans-serif);font-size:13px;font-weight:600;padding:10px 34px;border:none;border-radius:10px;cursor:pointer;transition:all 0.35s cubic-bezier(0.23,1,0.32,1);color:rgba(255,255,255,0.3);background:transparent;position:relative;overflow:hidden}
        ...
        .slbl{font-family:var(--font-primary,'Open Sans',sans-serif);font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:3.5px;color:rgba(255,255,255,0.15);margin-bottom:16px;position:relative;padding-left:12px}
        ...
        .stat-val{font-family:var(--font-primary,'Open Sans',sans-serif);font-size:30px;font-weight:800;transition:all 0.3s;margin-bottom:4px;animation:countUp 0.5s ease forwards}
        .stat-lbl{font-size:10px;color:rgba(255,255,255,0.2);text-transform:uppercase;letter-spacing:2px;font-weight:600}
        ...
        .mem-av{width:42px;height:42px;border-radius:12px;background:#0a0a0a;border:2px solid rgba(255,96,64,0.25);display:flex;align-items:center;justify-content:center;font-family:var(--font-primary,'Open Sans',sans-serif);font-size:16px;font-weight:700;color:#ff6040;transition:all 0.35s;overflow:hidden;flex-shrink:0;cursor:pointer}
        .mem-av img{width:100%;height:100%;object-fit:cover;border-radius:10px}
        .profile-av{width:84px;height:84px;border-radius:22px;background:#0a0a0a;border:2.5px solid rgba(255,96,64,0.35);display:flex;align-items:center;justify-content:center;font-family:var(--font-primary,'Open Sans',sans-serif);font-size:34px;font-weight:800;color:#ff6040;flex-shrink:0;box-shadow:0 8px 32px rgba(255,48,32,0.15);transition:all 0.35s;animation:glowPulse 3s ease-in-out infinite;overflow:hidden;cursor:pointer}
        ...
        @media(max-width:900px){.mem-grid{grid-template-columns:repeat(2,1fr)!important}}
        @media(max-width:768px){.mem-grid{grid-template-columns:1fr!important}.stats-grid{grid-template-columns:repeat(2,1fr)!important}.info-grid{grid-template-columns:1fr!important}}
      `}</style>
      ...
      {fullPhoto && (
        <div className="photo-overlay" onClick={function () { setFullPhoto(null) }}>
          ...
          <div style={{ fontFamily: "var(--font-primary,'Open Sans',sans-serif)", fontSize: 24, fontWeight: 800, color: "#fff", letterSpacing: 1.5 }}>{fullPhoto.name}</div>
      ...
      <div className="scroll-area" style={{ flex: 1, padding: "36px 48px 80px", overflowY: "auto", maxHeight: "100vh" }}>
        ...
        <div className="anim d1" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32 }}>
          <h1 style={{ fontFamily: "var(--font-primary,'Open Sans',sans-serif)", fontSize: 28, fontWeight: 800, letterSpacing: 0.5 }}>Dashboard</h1>
        ...
        <div className={"tabcontent " + (tab === "team" ? "on" : "")}>
          ...
          <div className="glass anim d3" style={{ padding: "40px 36px", marginBottom: 20, position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ fontFamily: "var(--font-primary,'Open Sans',sans-serif)", fontSize: 11, textTransform: "uppercase", letterSpacing: 3, color: "rgba(255,255,255,0.2)", marginBottom: 10 }}>Project</div>
            <h2 style={{ fontFamily: "var(--font-primary,'Open Sans',sans-serif)", fontSize: 34, fontWeight: 800, lineHeight: 1.2, marginBottom: 14 }}>{team.project_title}</h2>
          ...
          <div className="anim d5" style={{ marginBottom: 24 }}>
            <div className="slbl">Project Milestones</div>
            <div className="glass" style={{ padding: "8px 20px 28px", overflow: "hidden" }}>
              <MilestoneTimeline
                teamNumber={teamNumber}
                technology={teamTechnology}
                isLeader={isLeader}
              />
            </div>
          </div>
                ...
                return (
                  <div key={m.id} className="glass mem-card">
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                      <div className="mem-av" onClick={function (e) {
                        e.stopPropagation()
                        var url = getPhotoUrl(m.member_roll_number, m.member_college)
                        if (url) setFullPhoto({ url, name: m.member_name, roll: m.member_roll_number, college: m.member_college })
                      }}>
                        {getPhotoUrl(m.member_roll_number, m.member_college)
                          ? <img src={getPhotoUrl(m.member_roll_number, m.member_college)} alt={m.member_name} onError={function (e) { e.target.style.display = "none"; e.target.nextSibling && (e.target.nextSibling.style.display = "flex") }} />
                          : null}
                        <span style={{ display: getPhotoUrl(m.member_roll_number, m.member_college) ? "none" : "flex" }}>{m.member_name.charAt(0).toUpperCase()}</span>
                      </div>
                      <div>
                        <div style={{ fontFamily: "var(--font-primary,'Open Sans',sans-serif)", fontSize: 16, fontWeight: 700, color: "#fff" }}>{m.member_name}</div>
                        {m.is_leader && <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1.5, color: "#ff6040" }}>Leader</div>}
                      </div>
                    </div>
        ...
        <div className={"tabcontent " + (tab === "you" ? "on" : "")}>
          ...
          <div className="glass anim d3" style={{ padding: 36, marginBottom: 20, display: "flex", gap: 24, alignItems: "center", position: "relative", overflow: "hidden" }}>
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: "linear-gradient(90deg,#ff3020,#ff6040,transparent)" }} />
            <div className="profile-av" onClick={function () {
              if (currentMember) {
                var url = getPhotoUrl(currentMember.member_roll_number, currentMember.member_college)
                if (url) setFullPhoto({ url, name: userName, roll: loggedInRoll, college: currentMember.member_college })
              }
            }}>
              {currentMember && getPhotoUrl(currentMember.member_roll_number, currentMember.member_college)
                ? <img src={getPhotoUrl(currentMember.member_roll_number, currentMember.member_college)} alt={userName} onError={function (e) { e.target.style.display = "none" }} />
                : null}
            ...
            <div>
              <h2 style={{ fontFamily: "var(--font-primary,'Open Sans',sans-serif)", fontSize: 30, fontWeight: 800, marginBottom: 4 }}>{userName}</h2>
              {isLeader && <div style={{ display: "inline-block", fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, padding: "3px 10px", borderRadius: 5, background: "rgba(255,96,64,0.08)", border: "1px solid rgba(255,96,64,0.15)", color: "#ff6040", marginBottom: 8 }}>Team Leader</div>}
            ...
            {[
              ...
              return (
                <div key={i} className="glass" style={{ padding: 18 }}>
                  <div style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: 2, color: "rgba(255,255,255,0.15)", marginBottom: 6 }}>{info.label}</div>
                  <div style={{ fontFamily: "var(--font-primary,'Open Sans',sans-serif)", fontSize: 16, fontWeight: 600, color: "rgba(255,255,255,0.75)" }}>{info.value}</div>
                </div>
              )
            })}
          </div>
              ...
              {(team.technologies || []).map(function (tech, i) {
                ...
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, borderRadius: 12, background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.04)", padding: paddings[sz] }}>
                    ...
                    <span style={{ fontWeight: sz === "xl" ? 700 : 500, fontSize: fontSizes[sz], color: sz === "xl" ? "rgba(255,255,255,0.8)" : sz === "lg" ? "rgba(255,255,255,0.65)" : sz === "md" ? "rgba(255,255,255,0.45)" : "rgba(255,255,255,0.3)", fontFamily: "var(--font-primary,'Open Sans',sans-serif)" }}>{tech}</span>
                  </div>
                )
              })}
            </div>
            ...
            <div className="glass" style={{ padding: 24 }}>
              <p style={{ fontSize: 13.5, lineHeight: 1.8, color: "rgba(255,255,255,0.45)", maxWidth: 700 }}>
                {team.project_description
                  ? "Working on " + team.project_title + " — " + team.project_description.substring(0, 200) + (team.project_description.length > 200 ? "..." : "")
                  : "Team member at Project Space."}
              </p>
            </div>
          </div>

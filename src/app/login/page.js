...
function LoginPage() {
  ...
  var simpleMode = searchParams.get("simple") === "1"

  var [role, setRole] = useState(simpleMode ? "simple" : "")
    ...
    fetch("/api/set-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "check_roll", rollNumber: rollNumber.trim().toUpperCase() }) })
      ...
      .then(function (data) {
        if (data.success) { setMemberEmail(data.email || ""); toast.success("OTP sent to " + (data.email || "your email")); setSetupStep("otp") }
          ...
          if (data.status === "not_registered") { toast.error("You're not registered in any team yet.") }
          else if (data.status === "already_exists") { toast.error("You already have a password. Please login."); setMode("login"); setPassword("") }
  ...
  function getSubtitle() {
    if (simpleMode) return "Welcome back — log in to continue"
  ...
  return (
    <div className="nx-page">
      ...
      <style jsx>{`
        ...
        @keyframes nxShoot1 { 0%{top:8%;left:-5%;opacity:0} 2%{opacity:1} 8%{top:28%;left:35%;opacity:0} 100%{opacity:0} }
        @keyframes nxShoot2 { 0%{top:5%;left:40%;opacity:0} 1.5%{opacity:0.8} 6%{top:22%;left:72%;opacity:0} 100%{opacity:0} }
        @keyframes nxShoot3 { 0%{top:15%;left:60%;opacity:0} 1%{opacity:0.6} 5%{top:30%;left:88%;opacity:0} 100%{opacity:0} }
        ...
        @keyframes nxArcRise { 0%{transform:translateY(40%);opacity:0} 15%{opacity:0.4} 40%{opacity:0.8} 100%{transform:translateY(0%);opacity:1} }
        ...
        @keyframes nxExpand { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:translateY(0)} }
      ...
      <div className="nx-wrap">
        <div className="nx-card">
          ...
          {/* ===== SIMPLE MODE — no roles, just login ===== */}
          {simpleMode && (
            <div className="nx-form-area" key="simple-login">
              <form className="nx-form" onSubmit={handleLogin}>
                ...
                <div className="nx-pw-wrap">
                  <input type={showPassword ? "text" : "password"} className="nx-input" placeholder="Password" value={password} onChange={function (e) { setPassword(e.target.value) }} />
                  <button type="button" className="nx-pw-btn" onClick={function () { setShowPassword(!showPassword) }}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={showPassword ? "#ff6040" : "rgba(255,255,255,0.2)"} strokeWidth="1.5"><path d="M1 12S5 5 12 5s11 7 11 7-4 7-11 7S1 12 1 12z"/><circle cx="12" cy="12" r="3"/></svg>
                  </button>
                </div>
              ...
              <div className="nx-link">Switch to role login? <a href="/login">Click here</a></div>
            </div>
          )}

          {/* ===== NORMAL MODE — role cards ===== */}
          {!simpleMode && (
            <>
                  ...
                  {setupSuccess ? (
                    ...
                    <>
                      <div className="nx-steps">
                        ...
                        <div className="nx-dline" />
                        <div className={"nx-dot" + (setupStep === "otp" ? " on" : setupStep === "password" ? " dn" : "")} />
                        <div className="nx-dline" />
                        <div className={"nx-dot" + (setupStep === "password" ? " on" : "")} />
                      </div>
                      {setupStep === "roll" && (
                      ...
                      {setupStep === "password" && (
                        <form className="nx-form" onSubmit={handleCreatePassword}>
                          ...
                          <div className="nx-pw-wrap">
                            <input type={showPassword ? "text" : "password"} className="nx-input" placeholder="Create Password" value={password} onChange={function (e) { setPassword(e.target.value) }} autoFocus />
                            <button type="button" className="nx-pw-btn" onClick={function () { setShowPassword(!showPassword) }}>
                              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={showPassword ? "#ff6040" : "rgba(255,255,255,0.2)"} strokeWidth="1.5"><path d="M1 12S5 5 12 5s11 7 11 7-4 7-11 7S1 12 1 12z"/><circle cx="12" cy="12" r="3"/></svg>
                            </button>
                          </div>
